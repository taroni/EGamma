...
#    queue="8nh"
    batchdir="$(pwd)/log_and_results2/newMC"
    mkdir -p $batchdir
...
#    for myfile in `/afs/cern.ch/project/eos/installation/0.3.84-aquamarine/bin/eos.select ls store/data/$dataset/000/${pathrunnb}/00000` 
    ...
    for myfile in `/afs/cern.ch/project/eos/installation/0.3.84-aquamarine/bin/eos.select ls /store/user/ndev/MinBias-Gen-Sim/Nab_raw2digi/160310_200606/0001`
     do
      echo $myfile 
...
#      cp -r configuration/configTPGtemplate.py $batchdir/runTPG_cfg_${s}.py
      cp -r template.py $batchdir/runTPG_cfg_${s}.py
      cp -r batch_template_AAA.sh $batchdir/batch_job_${s}.sh

      cd $batchdir/
      sed -e "s,%globaltag%,$gtag,"        \
          -e "s,%numevents%,$eventsPerCastorFile,"        \
	  -e "s,%filename%,root://eoscms//eos/cms/store/user/ndev/MinBias-Gen-Sim/Nab_raw2digi/160310_200606/0001/$myfile,"      \
	  -e "s,%isMC_%,$isMC," \
          -i runTPG_cfg_${s}.py
