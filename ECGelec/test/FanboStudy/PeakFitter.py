...
def doubleCBFit(histo, rangeInSigma=2., fitrebin=1):
    ...
    rms = fit.GetParameter(2)
    norm = fit.GetParameter(0)
    fit = ROOT.TF1("doubleCBFit", DoubleCB, mean-rms*(rangeInSigma-1), mean+rms*(rangeInSigma+1), 7)
    ...
    #fit = ROOT.TF1("doubleCBFit", Cruijff, histoCopy.GetBinLowEdge(1), histoCopy.GetBinLowEdge(histoCopy.GetXaxis().GetLast())+histoCopy.GetBin(histoCopy.GetXaxis().GetLast()),6 )
    #fit.SetParLimits(1, 0.5, 1.5)
    #fit.SetParLimits(2, 0.001, 1.)
    fit.SetParLimits(3, 0.01, 10.)
    fit.SetParLimits(4, 0., 100.)
    fit.SetParLimits(5, 0.01, 10.)
    ...
    #fit.SetParLimits(4, rms/rangeInSigma, rms*rangeInSigma)
    #fit.SetParLimits(5, 0.001, 10.)
    
    fit.SetParLimits(4, 0.1, 110.)
    ...
    histoCopy.Fit(fit, "WLE"," ",mean-rangeInSigma*rms, mean+rangeInSigma*rms)
