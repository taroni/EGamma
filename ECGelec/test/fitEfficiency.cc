...
int main(int argc, char**argv){
  ...
  mydir.str("");
  mydir << "root://eoscms//eos/cms/store/caf/user/taroni/TestTriggerOptimization2015/";
  ...
  fitManyFiles(30, 0, 1, 0, 1,  myfilevector,
	       "FGVB ratio theshold 0.90", "FGVB ratio threshold 0.95", mydir.str(), "1 fb^{-1}", 4, 
	       kBlack, kFullCircle, kRed, kOpenSquare, "WP80", "WP80");
  app.Run(); 
...
void fastEfficiencySilvia(unsigned int iEG, int iECAL1, int iColl1, int iECAL2, int iColl2, 
			  TString fileIn0="effi_TagProbe_tree_*.root",TString fileIn1="effi_TagProbe_tree_changed.root", 
  ...
  RooRealVar mean("mean","mean",float(iEG),0,100);
  ...
  RooRealVar mean2("mean2","mean",float(iEG),0,100);
  ...
  double fit_cuts_min = thres[iEG]-5. ;
  ...
  frame->GetXaxis()->SetRangeUser(1,100.);// set to 1000 to see the plateau
...
void fitManyFiles(unsigned int iEG, int iECAL1, int iColl1, int iECAL2, int iColl2, 
  ...
  // PARAMETRES ROOFIT CRYSTAL BALL
    RooRealVar norm("norm","N",0.1,0.05,1);
    RooRealVar alpha("alpha","#alpha",2.,0.1,8);
    RooRealVar n("n","n",5.,1.1,35);
    RooRealVar mean("mean","mean",float(thres[iEG]),15,1000);
    //mean.setVal(thres[iEG]);
    RooRealVar sigma("sigma","#sigma",1.5,0.1,5);
    //RooRealVar pedestal("pedestal","pedestal",0.01,0,0.4);
    if (DEBUG) cout << __LINE__ << endl;
   ...
   cb.plotOn(frame,LineColor(i+1),LineWidth(2));
    ...
    name_image.str("");
    name_image << dirResults.str() << "eff_EG"<<names[iEG]<<"_tag"<<tag<<"_probe"<<probe << "_fit_"<<name.c_str()<< ".root" ; 
    if (DEBUG) cout << __LINE__ << endl;
