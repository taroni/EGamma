...
void selectPairs(string myfile, int nEntries, int iEff, int iEff_M, TString eg_menu, string dirOut, string dirIn, TString tagCuts, double tagEt, TString probeCuts, double probeEt, double massCut1, double massCut2, TString tag_hlt, TString nameChain, bool checkCharge, bool data2011, bool debug);

using namespace std;
string myjobid, inputfile;
//-------
int main(int argc, char**argv){
  ...
  inputDir.str("");
  outputDir.str(""); 
  inputDir << "root://eoscms//eos/cms/store/caf/user/taroni/TestTriggerOptimization2015/"<< myjobid<<"/"<< "makePairsDir";
  
  //inputDir << myjobid<<"/"<<"makePairsDir";
  outputDir<< myjobid<<"/"<<"selectPairsDir";
  selectPairs(inputfile, -1, 0, 0, "L1_EG_MENU_2011",
	      outputDir.str(),
	      inputDir.str(),
	      "WP80", 20, "WP95", 5, 
...
//------
void selectPairs(string inputfile, int nEntries=-1, int iEff=0, int iEff_M=0, TString eg_menu="L1_EG_MENU_2011",
		 ...
		 bool checkCharge=true, bool data2011=true, bool debug=false)
{
  ...
  for (int ifile=0; ifile<filenames.size() ; ifile++){
    ...
    stringstream myfile; 
    myfile.str("");
    myfile << dirIn<<"/elepairs_"<< filenames[ifile] ;
    // myfile << "elepairs_" << ifile << ".root"; 
   
    TString file = myfile.str().c_str();
