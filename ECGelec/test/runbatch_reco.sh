...
cmsMkdir /store/caf/user/ndev/TPG/${runnb}
...
#    queue="8nh"
    batchdir="$(pwd)/log_and_results/${runnb}-batch"
    mkdir -p $batchdir
    ...
    for myfile in `./das_client.py --query="file dataset=${dataset} run=${runnb}" --limit=0`
      ...
      cp -r reco_RAW2DIGI_L1Reco_RECO.py $batchdir/runTPG_cfg_${s}.py
      cp -r batch_template_RAWRECO.sh $batchdir/batch_job_${s}.sh
