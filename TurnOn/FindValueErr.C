...
//  TString name_h[nEff];
  ...
  TH1D * hEff = new TH1D("hEff","hEff",10000, 0, 1.1);
  TH1D * hEt[nEff];

  for(int iEff=0 ; iEff<nEff ; iEff++)
  {  TString S1="hEt_"+name_h[iEff];
     hEt[iEff] = new TH1D(S1,S1,10000, 0, 150);
  }
  double ref=0 ;
  ...
  for(int iEff=0 ; iEff<nEff ; iEff++) {
    ...
    //ref_et[iEff] = dichotomie(eff[iEff],0,20000,0.0000001,cb,verbose);
   // ref_et[iEff] = dichotomie(eff[iEff],0,4000,0.000000001,cb,verbose);
    ref_et[iEff] = dichotomie(eff[iEff],0,10000,0.01,cb,verbose);
  ...
  for (int i=0 ; i<nbdata ; i++) {
    if (i%5000==0 && verbose) cout << "=== " << i << "th iteration" << endl;
//    if (i==2941)
//      continue;
//    if (i>354&&i<356)
//       continue;
...
//       continue;
    ...
    for(int iEff=0 ; iEff<nEff ; iEff++) {
      ...
      et_c[iEff] = dichotomie(eff[iEff],0,10000,0.01,cb,verbose);
...
int prodResults(TString dir="ratioScan/selectPairsDir/turnons/EG20/", TString file="eff_EG20_tagWP80_probeWP80_EB_N_vs_EE_N_fitres.root", TString namews="workspace", int nbdata=2400, double et_plateau=100, 
		bool draw=false, bool verbose=false) 
{

  // Define efficiency/et values and errors
  const int nEff=7; // # valeurs d'efficacite
  const int nE=2;   // EB / EE

//  double eff[nEff] = {0.25, 0.33, 0.5, 0.66, 0.75, 0.9, 0.95, 0.98, 0.99}; // efficacites a dichotomiser
  double eff[nEff] = {0.25, 0.33, 0.5, 0.66, 0.75, 0.9, 0.95}; // efficacites a dichotomiser
...
//  TApplication app("App",&argc, argv);
  ...
  prodResults("ratioScan/selectPairsDir/turnons/EG20/","eff_EG20_tagWP80_probeWP80_EE_N_vs_EE_N_fitres.root","workspace",10000,100,true,true);
